(async () => {
  ...
  try {
    ...
    r = await fetch(base + '/vejboder', {
      ...
      body: JSON.stringify({ navn: 'smoke-test', lat: 0.1, lng: 0.2, produkter: ['smoke'], ejer_id: 1 }),
    });
    j = await r.json();
    ...
    r = await fetch(base + '/vejboder/' + id, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ navn: 'smoke-updated', lat: 0.3, lng: 0.4, produkter: ['smoke','updated'] }),
    });
    j = await r.json();
